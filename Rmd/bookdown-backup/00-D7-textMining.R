...
filename <- "01-intro_Xmax.Rmd" # "01-amor-a.Rmd"
...
wordsTechnical <- c("the", "to", "a", "in", 
                    ...
                    "ta", "te", 
                    "en", "au", "ce", "est", "aux",
                    "1", "2", "3", "4", "pam", "pum", "x2", "x3")

# 2.1 * Extract and plot words ####


d7.getWords<- function(filename, MIN_COUNT=2) {
  ...
  dt_tidy_books %>%
    ...
    count(word) %>%
    with(wordcloud(word, n, min.freq=MIN_COUNT,
...
tidy_books <- d7.getWords(filename)

filename <- "backup/noel-lyrics-eng.Rmd"

filename <- "backup/noel-lyrics-fre.Rmd"
